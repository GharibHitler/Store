...
async function loadData() {
  try {
    ...
    GAMES = [
      {
        ...
        packages:[
          ...
          {id:2,amount:325, price_egp:260, badge:"",              hot:false},
          {id:3,amount:660, price_egp:505, badge:"",              hot:true},
          {id:4,amount:1845,price_egp:1260,badge:"الأكثر مبيعاً", hot:true},
          {id:5,amount:3940,price_egp:2500,badge:"خصم",           hot:false},
          {id:6,amount:8280,price_egp:5000,badge:"VIP",           hot:false},
        ],
        prosperity:[
    ...
    POPUPS = {
      ...
      purchases:["660 UC — 505 جنيه","325 UC — 260 جنيه","583 جوهرة — 245 جنيه","788 Coin — 370 جنيه","PRIME المميزة — 485 جنيه"]
    };
  }
}
...
function renderPackages(containerId, game) {
  ...
  grid.innerHTML = game.packages.map((p, i) => {
    ...
    // سعر الـ UC per جنيه للعرض
    const ucPerEgp = (p.amount / p.price_egp).toFixed(2);

    let iconHTML = "";
    if (isPubg) {
      iconHTML = `<svg class="pkg-uc-icon" width="40" height="40" viewBox="0 0 48 48" fill="none">
        <circle cx="24" cy="24" r="20" fill="#C9A84C" opacity="0.12"/>
        <polygon points="24,7 29,20 43,20 32,28 36,42 24,33 12,42 16,28 5,20 19,20" fill="#C9A84C"/>
      </svg>`;
    } else if (isFF) {
      iconHTML = `<span class="pkg-emoji" style="font-size:2.2rem;display:block;margin:0 auto 4px">💎</span>`;
    } else {
      iconHTML = `<span class="pkg-emoji" style="font-size:2.2rem;display:block;margin:0 auto 4px">🪙</span>`;
    }
    ...
    <div class="pkg-card${p.hot ? " hot" : ""} reveal" style="transition-delay:${i * .05}s;--accent:${accentColor}">
      ...
      <div class="pkg-top">
        ...
        <div class="pkg-rate-hint">${ucPerEgp} ${unitLabel} / جنيه</div>
      </div>
      <div class="qty-wrap">
      ...
      <div class="pkg-price-box">
        ...
        <div class="pkg-price-note">للوحدة الواحدة</div>
      </div>
...
function getCalcPkgs()      { return GAMES.find(g => g.id === calcGame)?.packages || []; }
function getCalcCurrency()  { return GAMES.find(g => g.id === calcGame)?.currency || "UC"; }
function getCalcGameLabel() { return GAMES.find(g => g.id === calcGame)?.label || ""; }
...
function calcMoney() {
  ...
  if (combo.length === 0) {
    const minPkg = [...pkgs].sort((a,b) => a.price_egp - b.price_egp)[0];
    resultEl.innerHTML = `
      <div class="calc-placeholder">
        <span>💸</span>
        <span>المبلغ لا يكفي لأي باقة<br/><small style="color:var(--muted)">أقل باقة بـ ${minPkg.price_egp} جنيه</small></span>
      </div>`;
    return;
  }
